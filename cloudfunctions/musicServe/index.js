...
exports.main = async(event, context) => {
	if(event.flag == 'search'){
		const data = {
			s: event.keywords,
			type: event.type || 1, // 1: 单曲, 10: 专辑, 100: 歌手, 1000: 歌单, 1002: 用户, 1004: MV, 1006: 歌词, 1009: 电台, 1014: 视频
			limit: event.limit || 30,
			offset: event.offset || 0
		}

		return createRequest(
			'POST', `https://music.163.com/weapi/search/get`, data,
			{ crypto: 'weapi', cookie: event.cookie, proxy: event.proxy }
		)
	}else if(event.flag =='lyric'){
		const data = {
			id: event.id
		}
		return createRequest(
			'POST', `https://music.163.com/weapi/song/lyric?lv=-1&kv=-1&tv=-1`, data,
			{ crypto: 'linuxapi', cookie: event.cookie, proxy: event.proxy }
		)
	}
